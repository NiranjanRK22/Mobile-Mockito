...
//		use the proxy object to call the method of IMobileService with when then
	...
	void testOrderInvalid() throws MobileNotFoundException  {
		...
		when(mobileService.getById(10)).thenReturn(null);
		String actualMobile = orderDetails.orderMobile(10);
		assertEquals(expected, actualMobile, "values are different");
	}
	
	@Test
	void testOrderException() throws MobileNotFoundException  {
		String expected = "mobile not ordered";
		when(mobileService.getById(10)).thenThrow(MobileNotFoundException.class);
